// test/delivery-skill-integration.test.mjs — the builder-skill delivery contract across the
// devtools command set: build.mjs (parseBuildArgs / resolveBuilderSkillContext /
// buildImplementPrompt / buildCodeReviewPrompt), ship.mjs (parseShipArgs / buildPlanPrompt /
// buildPlanReviewPrompt / builderSkillCheckpoint) and spec-eval.mjs (evaluateSpec rejects
// unknown skill declarations before the adversarial model is called).
//
// AIO-662 (moved from aios-workspace): every module under test is devtools. The `repo` these
// take is a WORKSPACE (it must carry .claude/skill-suite.json and the delivery skill bodies —
// core-owned content), so a minimal fixture workspace is staged from an AIOS toolkit checkout
// at runtime and every test skips with a named reason without one
// (docs/devtools-toolkit-contract.md; same pattern as spec-eval-adversarial.test.mjs).
...
test(
  "build accepts repeatable builder skills and spec declarations when flags are absent",
  { skip: SKIP },
  () => {
...
test(
  "explicit builder flags override spec declarations and full bodies reach only builder prompts",
  { skip: SKIP },
  () => {
...
test(
  "ship parses repeatable flags and separates plan bodies from reviewer audit pointers",
  { skip: SKIP },
  () => {
...
test("resume checkpoint changes when any selected skill hash changes", { skip: SKIP }, () => {
  const context = resolveBuilderSkillContext({
    repo: REPO,
    plan: "# Plan",
    builderSkills: ["evolve-versioned-contract"],
...
test(
  "unknown spec declarations fail before the adversarial model is called",
  { skip: SKIP },
  async () => {
    let calls = 0;
...
test(
  "declarations can be validated from raw issue frontmatter when the evaluated spec has a heading",
  { skip: SKIP },
  async () => {
    let calls = 0;
    await assert.rejects(
...
test(
  "publishable evaluation refuses SPEC_READY when the evaluated tree is dirty",
  { skip: SKIP },
  async () => {
...
test("a clean NOT_READY evaluation is never publishable", { skip: SKIP }, async () => {
  const result = await evaluateSpec({
    specText: STRONG,
    repo: REPO,
    rubric: RUBRIC,
...
test("legacy no-skill plans preserve the old prompt shape", { skip: SKIP }, () => {
