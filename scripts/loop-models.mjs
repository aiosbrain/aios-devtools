...
export function resolveLoopModels({ configPath, repo, cliOverrides = {}, profile = null } = {}) {
  ...
  for (const step of STEPS) {
    ...
    const cliValue = over.model ?? resolveReviewerPreset(over.preset)?.model;
    const profValue = prof.model ?? resolveReviewerPreset(prof.preset)?.model;
    const fileValue =
      fileCfg[`${step}_model`] ?? resolveReviewerPreset(fileCfg[`${step}_preset`])?.model;
